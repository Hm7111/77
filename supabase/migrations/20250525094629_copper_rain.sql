/*
  # نظام المهام الإلكترونية

  1. الجداول الجديدة
     - `tasks` - جدول المهام الرئيسية
     - `task_logs` - سجل تحديثات المهام والملاحظات
     - `task_attachments` - مرفقات المهام (اختياري)
  
  2. الأمان
     - تفعيل نظام RLS لكل الجداول
     - إنشاء سياسات لضمان عدم وصول المستخدمين إلا للمهام المصرح بها
     
  3. الوظائف والمشغلات
     - إنشاء وظيفة لتسجيل تغييرات المهام
     - إنشاء مشغل لتحديث حالة المهمة تلقائياً
*/
...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    IF OLD.status IS DISTINCT FROM NEW.status THEN
      ...
      VALUES (
        NEW.id, 
        COALESCE(current_setting('request.jwt.claims', true)::json->>'sub', NEW.created_by),
        'update_status',
        OLD.status,
        NEW.status,
        CASE
...
CREATE POLICY users_view_own_task_logs
  ...
  USING (
    EXISTS (
      ...
      AND (tasks.created_by = uid() OR tasks.assigned_to = uid())
    )
  );
