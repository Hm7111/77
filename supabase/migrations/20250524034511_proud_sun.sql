/*
  # إصلاح وظائف الموافقة وإضافة وظائف جديدة

  1. تحسينات
    - تحسين وظيفة الموافقة على الخطابات
    - تحسين وظيفة رفض الخطابات
    - إضافة وظيفة للحصول على تفاصيل الخطاب من طلب الموافقة
    - إضافة وظيفة للتحقق من صحة طلب الموافقة

  2. إصلاحات
    - إصلاح مشكلة معرف طلب الموافقة غير صالح
    - إصلاح مشكلة عدم ظهور رمز QR في طلبات الموافقة
    - تحسين التعامل مع الأخطاء وتسجيلها
*/

-- وظيفة للحصول على معرف الخطاب من طلب الموافقة
CREATE OR REPLACE FUNCTION get_letter_by_request_id(
  p_request_id UUID
) RETURNS TABLE (
  letter_id UUID,
  user_id UUID
...
-- وظيفة للحصول على تفاصيل الخطاب للموافقة
CREATE OR REPLACE FUNCTION get_letter_details_for_approval(
  p_letter_id UUID
) RETURNS TABLE (
  letter_id UUID,
  template_id UUID,
